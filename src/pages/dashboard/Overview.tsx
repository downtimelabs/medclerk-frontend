      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-6">
            ...
            {activeDoctors.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {activeDoctors.map((doctor) => (
                  <Card key={doctor.id} className="p-4 flex items-center gap-4 hover:shadow-md transition-all cursor-pointer group border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                    <div className="relative">
                      <div className="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 font-bold text-lg border border-slate-100 dark:border-slate-600 group-hover:border-[#0277BD] transition-colors">
                ...
                <Card className="flex flex-col items-center justify-center gap-2 border-dashed border-slate-300 dark:border-slate-600 bg-slate-50/50 dark:bg-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800 hover:border-[#0277BD] dark:hover:border-blue-400 cursor-pointer transition-all min-h-[88px]" onClick={() => navigate('/patient/doctors')}>
          ...
          <section>
            ...
            <Card className="p-0 overflow-hidden border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
              ...
              <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                {documentStats?.documentsByType && Object.entries(documentStats.documentsByType).map(([type, count], idx) => {
                  const styles = [
                    { border: 'border-blue-100', bg: 'hover:bg-blue-50/80', icon: 'text-blue-600', fill: 'bg-blue-100' },
                    { border: 'border-indigo-100', bg: 'hover:bg-indigo-50/80', icon: 'text-indigo-600', fill: 'bg-indigo-100' },
                    { border: 'border-teal-100', bg: 'hover:bg-teal-50/80', icon: 'text-teal-600', fill: 'bg-teal-100' },
                    { border: 'border-slate-100', bg: 'hover:bg-slate-50/80', icon: 'text-slate-600', fill: 'bg-slate-100' },
                  ];
        ...
        <div className="space-y-6">
          ...
          <section>
            ...
            <Card className="p-0 overflow-hidden border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg shadow-slate-200/50">
              <div className="p-5 space-y-5">
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-4">
                  <span className="text-sm text-slate-500 font-medium">Blood Group</span>
                  <span className="text-lg font-bold text-slate-900 dark:text-white bg-red-50 text-red-600 px-3 py-1 rounded-lg">
                ...
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-4">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-slate-500">{profile?.medical?.weightKg ? `${profile.medical.weightKg} kg` : '-'}</div>
                  </div>
                </div>
                <div className="space-y-2">
                  <span className="text-xs uppercase font-bold text-slate-400 tracking-wider">Known Conditions</span>
                  <div className="flex flex-wrap gap-2">
                    {(profile?.medical?.chronicConditions?.length || 0) > 0 ? (
                      profile?.medical?.chronicConditions.map((condition, i) => (
                        <span key={i} className="px-2.5 py-1 bg-teal-50 text-teal-700 text-xs font-semibold rounded-md border border-teal-100">
                          {condition}
                        </span>
                      ))
                    ) : (
                ...
                <div className="space-y-2">
                  <span className="text-xs uppercase font-bold text-slate-400 tracking-wider">Allergies</span>
                  <div className="text-sm font-medium text-slate-700 dark:text-white bg-slate-50 p-2 rounded-lg border border-slate-100">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="p-4 border-slate-200 dark:border-slate-700 bg-white hover:border-blue-300 transition-colors group">
                <div className="mb-2 w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors">
                  <Plus size={16} />
                </div>
                <div className="text-2xl font-bold text-slate-900 dark:text-white">{documentStats?.recentUploads || 0}</div>
                <div className="text-xs text-slate-500 font-medium">New Uploads</div>
              </Card>
              <Card className="p-4 border-slate-200 dark:border-slate-700 bg-white hover:border-orange-300 transition-colors group">
                <div className="mb-2 w-8 h-8 rounded-lg bg-orange-50 text-orange-600 flex items-center justify-center group-hover:bg-orange-600 group-hover:text-white transition-colors">
                  <FileText size={16} />
                </div>
